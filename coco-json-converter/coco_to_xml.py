...
for imgId in imgIds:
    ...
    for annId in annIds:
        ...
        cat = CLASSES[cat_id-1] #coco.loadCats(ann['category_id'])[0]
        
        if cat == 'background' and args.skip_background == True:
            continue
        
        box = ann['bbox']
        anno_list.append([cat, int(box[0]), int(box[1]), int(box[0]+box[2]), int(box[1]+box[3])])
